...
feature 'Create Email' do
  ...
  scenario 'has valid form' do
    ...
    expect(page).to have_field("email_body", with: "#{tutor.name} has invoiced #{invoice.hours} hours of tutoring for #{student.assignment.subject}. You have #{student.client.hourly_balance(student) <= 0 ? 0 : student.client.hourly_balance(student)} hours left in your hourly balance and payments must be made in advance before the next tutoring sessions.")
  end
